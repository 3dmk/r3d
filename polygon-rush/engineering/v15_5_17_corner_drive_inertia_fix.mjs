...
s=s.replace(combinedAnchor,`  const requestedLong=longForce;
  ...
  if(!front&&driveTorque>0&&!handbrake){
   const reservedLong=Math.min(Math.abs(requestedLong),maxForce*WHEEL_PHYS.drivenLongPriority);
...
for(const r of ['Polygon Rush v15.5.17 Corner Drive + Steering Response','yawInertiaScale:1.12','cornerCombinedGrip:1.28','drivenLongPriority:.90','steeringDriveAssist:.38','steeringYawRelease:2.4','rackRate=12.6','requestedLong','reservedLong','testCornerDriveResponse:()=>'])must(s.includes(r),'v15.5.17 missing '+r);
fs.writeFileSync(file,s);
console.log('Polygon Rush v15.5.17 corner drive + steering response applied');
