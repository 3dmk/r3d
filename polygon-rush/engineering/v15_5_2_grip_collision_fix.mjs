...
s=s.replace('cornerStiffness:7600, rollingResistance:42, aeroDrag:.46,','cornerStiffness:13200, rollingResistance:46, aeroDrag:.46,');
s=s.replace('steerMax:.58, steerResponse:7.5, yawDamping:1.55,','steerMax:.54, steerResponse:8.5, yawDamping:1.95, gripScale:1.62, lowSpeedGrip:2.4,');
s=s.replace('const maxF=Math.max(1200,normal*(1.15*grip));','const maxF=Math.max(1800,normal*(WHEEL_PHYS.gripScale*grip));');
s=s.replace('let latForce=-vLat*WHEEL_PHYS.cornerStiffness*(front?1.05:.95);','let latForce=-Math.tanh(vLat*0.72)*WHEEL_PHYS.cornerStiffness*(front?1.08:.98);');
s=s.replace('car.vel.x+=fx/WHEEL_PHYS.mass*dt;car.vel.z+=fz/WHEEL_PHYS.mass*dt;\n car.yawRate+=torqueY/WHEEL_PHYS.inertiaY*dt;',`car.vel.x+=fx/WHEEL_PHYS.mass*dt;car.vel.z+=fz/WHEEL_PHYS.mass*dt;
