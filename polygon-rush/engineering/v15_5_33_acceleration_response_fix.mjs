...
rep('wheelInertia:6.5, driveWheelTorque:2700, brakeWheelTorque:2600, longSlipStiffness:6.0, absSlip:.12, tcSlip:.24,','wheelInertia:5.4, driveWheelTorque:4200, brakeWheelTorque:2600, longSlipStiffness:6.6, absSlip:.12, tcSlip:.30,');
rep('tireMu:1.34, tireCorner:14500, tireLong:11800, drivetrain:1.14,','tireMu:1.38, tireCorner:14500, tireLong:13200, drivetrain:1.22,');
rep('const throttleRate=Math.abs(driveInput)>Math.abs(car._driveThrottle)?15.5:7.0;','const throttleRate=Math.abs(driveInput)>Math.abs(car._driveThrottle)?28.0:9.5;');
rep('const torqueCurve=clamp(1.20-Math.abs(vLong)/94,.46,1.12);','const launchBoost=1+.34*(1-clamp(Math.abs(vLong)/18,0,1));\n   const midPull=1+.18*(1-Math.abs(clamp((Math.abs(vLong)-24)/34,-1,1)));\n   const torqueCurve=clamp(1.42-Math.abs(vLong)/112,.56,1.34)*launchBoost*midPull;');
rep('const steerLoad=clamp(Math.abs(rawSteer),0,1),powerKeep=clamp(1-steerLoad*car._driveThrottle*.38,.72,1);','const steerLoad=clamp(Math.abs(rawSteer),0,1),powerKeep=clamp(1-steerLoad*Math.max(0,car._driveThrottle)*.20,.86,1);');

const hook='testObjectSurfaces:()=>';
