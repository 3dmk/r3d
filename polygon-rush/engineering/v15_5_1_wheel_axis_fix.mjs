...
// CylinderGeometry is born along local Y. Bake the 90-degree wheel orientation into
// the geometry itself so the mesh transform remains clean: pivot Y = steering,
...
replaceFunction('animateBuggy',`function animateBuggy(car,long,lat,dt){
 ...
 for(let i=0;i<wheels.length;i++){
  ...
  // Steering belongs only to the wheel pivot.
  ...
  // Geometry axle is baked to X; rolling therefore happens strictly around X.
...
// Extend runtime diagnostics so CI can verify the actual visual rotation axis.
s=s.replace("spin:(player.g?.userData?.allWheels||[]).map(w=>w.userData.spin||0),states:player._wheelPhysics?.length||0}:null})};",
            "spin:(player.g?.userData?.allWheels||[]).map(w=>w.userData.spin||0),wheelEuler:(player.g?.userData?.allWheels||[]).map(w=>w.userData.tire?[w.userData.tire.rotation.x,w.userData.tire.rotation.y,w.userData.tire.rotation.z]:[0,0,0]),states:player._wheelPhysics?.length||0}:null})};");
