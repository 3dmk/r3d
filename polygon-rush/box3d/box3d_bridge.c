...
API void pr_b3_apply_vehicle_control(int worldHandle,int bodyHandle,float throttle,float brake,float steer,float handbrake,float nitro,float grip,float forwardSpeed){ (void)worldHandle; b3BodyId body=body_id(bodyHandle); if(B3_IS_NULL(body))return; b3Quat q=b3Body_GetRotation(body); b3Mat33 m=b3MakeMatrixFromQuat(q); b3Vec3 forward=m.cz; b3Vec3 right=m.cx; b3Vec3 v=b3Body_GetLinearVelocity(body); float engine=9300.0f*(nitro>0.5f?1.25f:1.0f)*throttle; float braking=14000.0f*brake; b3Vec3 force=b3MulSV(engine,forward); if(fabsf(forwardSpeed)>0.5f && brake>0.0f){ force=b3Sub(force,b3MulSV(braking*(forwardSpeed>0?1.0f:-1.0f),forward)); } float lateral=b3Dot(v,right); float latDamp=(handbrake>0.1f?0.95f:5.2f)*grip; force=b3Sub(force,b3MulSV(lateral*1750.0f*latDamp,right)); b3Body_ApplyForceToCenter(body,force,true); float steerScale=fminf(fabsf(forwardSpeed)/8.0f,1.0f); float yawTorque=steer*steerScale*5000.0f*(handbrake>0.1f?1.20f:1.0f); b3Body_ApplyTorque(body,(b3Vec3){0,yawTorque,0},true); }
API float* pr_b3_get_body_state(int worldHandle,int bodyHandle){ (void)worldHandle; b3BodyId body=body_id(bodyHandle); if(B3_IS_NULL(body))return 0; float* out=(float*)malloc(sizeof(float)*7); if(!out)return 0; b3Pos p=b3Body_GetPosition(body); b3Vec3 v=b3Body_GetLinearVelocity(body); b3Quat q=b3Body_GetRotation(body); b3Mat33 m=b3MakeMatrixFromQuat(q); float yaw=atan2f(m.cz.x,m.cz.z); out[0]=p.x;out[1]=p.y;out[2]=p.z; out[3]=v.x;out[4]=v.y;out[5]=v.z; out[6]=yaw; return out; }
