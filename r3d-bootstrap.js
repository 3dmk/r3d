...
async function runRender(){if(busy)return;startPreview(true);busy=true;renderCanvas.dataset.r3dRasterDuringFinal='1';document.documentElement.dataset.r3dRasterDuringFinal='1';$('renderBtn').disabled=$('renderSide').disabled=true;const head=document.querySelector('.renderHead b');if(head)head.textContent='R3D Final Render';$('status').textContent='Final render computing • raster preview stays live';try{if(!window.R3DRenderer)throw new Error('R3DRenderer unavailable');await window.R3DRenderer.render()}catch(e){console.error('R3D render failed',e);$('status').textContent='Render failed: '+(e?.message||e)}finally{stopPreview();busy=false;$('renderBtn').disabled=$('renderSide').disabled=false;document.documentElement.dataset.r3dFinalRenderComplete='1'}}
...
document.documentElement.dataset.r3dRasterDuringFinal='0';
document.documentElement.dataset.r3dFinalRenderComplete='0';
document.documentElement.dataset.r3dBuild='1.0.0-rc3';
