...
async function runRender(){if(busy)return;stopPreview();busy=true;$('renderBtn').disabled=$('renderSide').disabled=true;const head=document.querySelector('.renderHead b');if(head)head.textContent='R3D Final Render';try{if(!window.R3DRenderer)throw new Error('R3DRenderer unavailable');await window.R3DRenderer.render()}catch(e){console.error('R3D render failed',e);$('status').textContent='Render failed: '+(e?.message||e)}finally{busy=false;$('renderBtn').disabled=$('renderSide').disabled=false}}
function cancel(){stopPreview();window.R3DRenderer?.cancel?.();$('status').textContent='Render cancelled'}
...
window.R3DRasterPreview={start:startPreview,stop:stopPreview,get active(){return previewActive}};
