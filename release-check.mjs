...
const files=['index.html','r3d-editor.js','r3d-renderer.js','r3d-core-validation.js','r3d-render-watchdog.js','r3d-feature-rays.js','r3d-input-priority-shim.js','r3d-local-orbit.js','r3d-camera-gizmos.js','r3d-gi-extension.js','r3d-atrous-denoiser.js','r3d-luma-quality.js','r3d-render-window.js','r3d-bootstrap.js'];
...
const expected=['r3d-editor.js?v=rc7','r3d-renderer.js?v=rc7','r3d-core-validation.js?v=rc7-corefix1','r3d-render-watchdog.js?v=rc7-orientfix1','r3d-feature-rays.js?v=rc7-feature1','r3d-input-priority-shim.js?v=rc7','r3d-local-orbit.js?v=rc7','r3d-camera-gizmos.js?v=rc7','r3d-gi-extension.js?v=rc7-gi5','r3d-atrous-denoiser.js?v=rc7-atrous1','r3d-luma-quality.js?v=rc7-luma2','r3d-render-window.js?v=rc7-debug1','r3d-bootstrap.js?v=rc7'];
...
const ed=read('r3d-editor.js'),ren=read('r3d-renderer.js'),core=read('r3d-core-validation.js'),watch=read('r3d-render-watchdog.js'),feature=read('r3d-feature-rays.js'),priority=read('r3d-input-priority-shim.js'),orbit=read('r3d-local-orbit.js'),cams=read('r3d-camera-gizmos.js'),gi=read('r3d-gi-extension.js'),atrous=read('r3d-atrous-denoiser.js'),lq=read('r3d-luma-quality.js'),rw=read('r3d-render-window.js'),boot=read('r3d-bootstrap.js');
...
for(const token of ['refineFeatureRays','scorePixel','r3dFeatureRayBudget','r3dFeatureRayCount','r3dFeatureRayPercent','Feature-guided full-resolution refinement','flat regions skipped'])if(!feature.includes(token))fail(`feature ray budget contract missing ${token}`);
if(!feature.includes("/CPU recovery/i"))fail('feature ray budget must remain scoped to CPU recovery until WebGPU output is validated');
...
for(const token of ['r3dGIControls','giEnabled','giStrength','giColorBounce','giColorStrength','giAO','giAOStrength','giAORadius','giAOSamples','giMix','giRecon','applyGI','hemiLD(','edgeAwareReconstruct','r3dGIComplete','r3dGIReconstruction','Edge-locked anti-bleed GI reconstruction','r3dGIAntiBleed','Edge-Locked High','localMin','localMax','polarity','confidence','edgeStrength'])if(!gi.includes(token))fail(`GI extension contract missing ${token}`);
if(gi.includes('w=bw*(.08+.92*edge)'))fail('GI reconstruction returned legacy cross-edge weight floor');
if(gi.includes("dataset.r3dGIReconstruction='edge-aware'"))fail('GI reconstruction returned legacy edge-aware marker');
if(/(?:^|[;{])\s*base\s*=\s*\[/.test(gi))fail('GI extension contains undeclared strict-mode base assignment');
for(const token of ['applyAtrousDenoise','buildGuide','varianceMap','fireflyClamp','atrousPass','r3dAtrousDenoiser','r3dAtrousPasses','r3dAtrousNoisyPercent','depth','normal','albedo','material','luminance','variance','reflectionStrict','steps:[1,2,4]','Feature-guided À-Trous denoise'])if(!atrous.includes(token))fail(`Atrous denoiser contract missing ${token}`);
if(!atrous.includes("$('denoise')?.value!=='1'"))fail('Atrous denoiser must respect Denoise Off');
for(const token of ['r3dLumaControls','lumaQuality','lumaShadow','lumaLight','lumaReflection','lumaDenoise','applyLumaQuality','r3dLumaKey','r3dLumaQuality','polarity-aware','r3dLumaAntiBleed','shadow/light boundary protected'])if(!lq.includes(token))fail(`luma quality contract missing ${token}`);
...
console.log(`staticIDs=${ids.length} scripts=${scripts.length} renderWindow=${rw.length} watchdogBasis=canonical coreValidation=clean-webgpu-retry featureRays=selective-fullres-budget gi=edge-locked-polarity-confidence-HQ atrous=feature-guided-3pass-depth-normal-albedo-material-variance luma=polarity-aware-anti-bleed debug=render-timeline+copy giMutableColor=verified`);