...
const expected=['r3d-editor.js?v=rc4','r3d-renderer.js?v=rc4','r3d-render-watchdog.js?v=rc4','r3d-output-orientation.js?v=rc4','r3d-progressive-pass.js?v=rc4','r3d-input-priority-shim.js?v=rc4','r3d-local-orbit.js?v=rc4','r3d-camera-gizmos.js?v=rc4','r3d-bootstrap.js?v=rc4'];
if(JSON.stringify(scripts)!==JSON.stringify(expected))fail('script load order/version changed');
if(!html.includes('v1.0 RC4'))fail('visible RC4 build marker missing');
...
for(const token of ['Progressive Ray Pass','r3dProgressivePass','progressiveActive','trace(','ray('])if(!prog.includes(token))fail(`progressive pass contract missing ${token}`);
...
for(const token of ["dataset.r3dBootstrap='1'",'progressive ray pass active',"dataset.r3dBuild='1.0.0-rc4'","dataset.r3dFinalRenderComplete='1'"])if(!boot.includes(token))fail(`bootstrap/progressive contract missing ${token}`);
